...
## [닫힘] (Goal 003) 예시 검증 데이터의 운영 D1·공개 아카이브 게시 승인 (2026-06-15)
- 처리: 사용자가 채팅으로 명시 승인(ADMIN_TOKEN 지목) → `seed-examples.sql`(votatis-data 9건, 검토 피드백 포함)을 운영 D1에 적용(`wrangler d1 execute votatis-reports --remote`). 검증: 운영 `/reports` 9건(confirmed 3/disputed 1/debunked 1/reviewing 2/unverified 2), 공개 상세에 피드백(status_scope/risk_level/not_confirmed/public_summary) 노출·`reviewer_note`는 비공개 유지, 라이브 `/archive`(런타임 API)에서 publishable 5건 표시.
- 주의: 예시 데이터의 evidence_links 는 example.com 플레이스홀더이고 reviewer 는 "검증자(예시)". 실데이터로 대체·정정하거나, 향후 정리(삭제)가 필요하면 동일 SQL 의 id 목록으로 DELETE.
- 후속(선택): GitHub 공개 data 레포 export(`export:data`)는 별도 [열림] 항목 참고. 지금 아카이브는 D1 런타임 조회라 export 없이도 노출됨.
