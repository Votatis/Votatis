...
- (해결되면) 이어서 진행할 것: 운영 검증 워크플로우 가동. **검증 관리 MCP(`votatis-admin-mcp`, spec 0014)도 이 토큰·운영 API URL을 `VOTATIS_ADMIN_TOKEN`/`VOTATIS_API_URL`로 받아 동작**한다(로컬은 dev-admin-token으로 이미 검증됨, `docs/mcp-usage.md`).
