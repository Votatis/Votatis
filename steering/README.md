...
| [drizzle-d1-schema-gotchas.md](drizzle-d1-schema-gotchas.md) | drizzle-orm 0.36(D1)에서 인덱스 extraConfig는 배열이 아니라 객체형 `(t)=>({...})`(배열형은 TS2769). 마이그레이션은 drizzle-kit generate→wrangler d1 migrations apply. ⚠️새 컬럼 참조 worker는 원격 마이그레이션을 배포보다 먼저(아니면 라이브 no such column 500). | pitfall, drizzle, d1, cloudflare | 2026-06-22 | 3 |
...
| [frontend-pages-deploy.md](frontend-pages-deploy.md) | apps/frontend SSG→Pages 배포: NEXT_PUBLIC_* 3개(API_BASE_URL/TURNSTILE_SITEKEY/R2_PUBLIC_URL) 빌드타임 주입(ELECTION은 spec0017부터 미사용→lib/elections.ts), `wrangler pages deploy out --branch=main`, preview origin은 CORS 막힘, Turnstile domains에 votatis-web.pages.dev 추가. | pitfall, deploy, cloudflare-pages, frontend, cors | 2026-06-21 | 10 |
...
| [admin-mcp-origin-gate.md](admin-mcp-origin-gate.md) | intake-api 는 /admin/* 모든 메서드를 ALLOWED_ORIGIN 으로 막음(cors.ts). /admin 부르는 서버-서버 호출자(admin-mcp=VOTATIS_ORIGIN, export-data.ts=ORIGIN)는 허용 Origin 헤더 보내야 통과, 안 보내면 전부 403. Node fetch 는 Origin 수동설정 가능. 운영 토큰은 --scope local 로(.mcp.json 커밋 금지). | pitfall, cors, admin-mcp, intake-api, mcp, origin | 2026-06-21 | 1 |
...
| [doc-tone-no-childish-analogies.md](doc-tone-no-childish-analogies.md) | 문서(온보딩/가이드)는 난이도는 쉽게 쓰되 유치한 비유(요리/도서관/공략 노트 등) 금지. "유치원생도 이해할 정도"=비유 넣으라는 게 아니라 난이도를 낮추라는 뜻. 평범·직접 설명 + 표/흐름도로. | convention, docs, writing, tone | 2026-06-21 | 1 |
| [static-list-ssr-not-client-fetch.md](static-list-ssr-not-client-fetch.md) | 정적(SSG) 페이지 목록을 클라 useEffect 로 채우면 초기 HTML 에 '불러오는 중'이 박히고, 배포 전파 중 stale 청크/캐시로 JS 안 돌면 로딩에서 영구 멈춤. 빌드타임에 아는 데이터는 서버 컴포넌트가 prop 주입해 SSG HTML 에 직접(필터 상호작용만 client). archive: page.tsx→ArchiveClient initial. | pitfall, frontend, nextjs, ssg, hydration, archive | 2026-06-21 | 1 |
| [frontend-buildtime-md-parse.md](frontend-buildtime-md-parse.md) | 프론트(Next16 Turbopack)에서 빌드타임에 repo data/*.md 를 서버컴포넌트로 파싱해 상세 렌더(archive-md.ts). 함정① js-yaml v5 default export 없음→`import { load as yamlLoad }`(default 쓰면 Turbopack 빌드 실패, typecheck는 통과). 함정② 경로는 `resolve(process.cwd(),"../../data")`. 인덱스는 슬림 요약, 상세 단일원천은 .md. | pitfall, frontend, nextjs, build, js-yaml, archive | 2026-06-21 | 1 |
