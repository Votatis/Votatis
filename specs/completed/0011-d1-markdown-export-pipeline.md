...
- 2026-06-21: export-data.ts 에 `ORIGIN` 환경변수(+ Origin 헤더) 추가 — /admin/* 가 cors.ts Origin 게이트로 막혀(Origin 미전송 시 403) 운영 export 가 불가하던 것 해결. 운영 D1의 검증완료 1건을 실제 export→마크다운+index.json 생성, 정적 빌드로 /archive/{id} 페이지(증거 이미지 R2 직접 로드 포함) 생성 확인. (요청: 채팅)
- 2026-06-21: 공개 아카이브 목록(/archive)을 런타임 /reports API 대신 빌드 정적 인덱스(archive.generated.json) 단일 원천으로 전환(loadArchiveSummaries). 목록 갱신은 export:data→build 재실행. 상세(record)는 런타임 조회 유지. (요청: 채팅)
- 2026-06-21: index.json/archive.generated.json 을 슬림 요약(목록·탐색·통계 필드만)으로 축소(레코드당 ~6KB→~0.5KB, 번들 비대화 방지). 상세 풀데이터의 단일 원천을 .md 로 두고, 정적 상세 /archive/[id] 가 빌드타임에 .md(frontmatter=js-yaml 파싱 + 본문 복원)를 읽어 렌더(archive-md.ts). 목록 카드 링크를 /archive/record?id=→/archive/[id](정적)로 전환. export-data 는 recordToSummary 로 index.json 생성. (요청: 채팅)
- 2026-06-21: /archive 목록을 서버 렌더(SSG)로 전환 — page.tsx 가 allSummaries() 를 ArchiveClient 에 prop 주입해 HTML 에 목록을 직접 담는다(기존 클라이언트 useEffect 로딩 제거). 배포 전파 중 stale 청크/캐시로 '불러오는 중'에서 멈추던 증상 해결. (요청: 채팅)
