...
**계정·권한 (spec 0015):** 루트 관리자(`admin`)가 검증자(회원) 계정을 생성하고 비밀번호 재설정 링크를 발급한다.
인증은 JWT 액세스 토큰(단명) + 회전형 리프레시 토큰(7일 미접속 시 만료). 검증 작업은 모든 회원이 가능하고,
회원 생성·삭제 등 계정 관리는 루트 전용. 운영 호환을 위해 `ADMIN_TOKEN` Bearer 는 루트 break-glass 로 유지.
...
- ✅ D1 스키마 및 입력 로직
- ✅ 관리자 검증 API + 인증 (검토 큐 조회, 상태 판정, 증거 열람) — spec 0008
- ✅ 관리자 콘솔 연동 (로그인, 검토 큐, 증거 검수·판정, 대시보드) — spec 0009
- ✅ 관리자 계정 인증 (루트 계정 + 회원관리, JWT access/refresh, 비밀번호 재설정 링크) — spec 0015
- ✅ 공개 데이터 배포 파이프라인 (D1 → 마크다운 export + 빌드타임 인덱스) — spec 0011
- ✅ 정적 사이트 (아카이브 목록·상세, 검색, 통계) — spec 0010
- ✅ AI 보조 분석 (자동 태그 추천·신뢰도 신호·합성 위험, 휴리스틱±Workers AI) — spec 0012

> 코드는 완성·검증(typecheck/test/static build)됐다. **운영 가동에 필요한 사람 작업**(운영 ADMIN_TOKEN·JWT_SECRET 발급,
> 원격 D1 마이그레이션, 운영 D1 대상 export 실행 + GitHub 커밋 + Pages 재빌드, 선택적 Workers AI 바인딩)은 `loops/HUMAN.md` 참고.

**계획 (v1 이후):**
- 벡터 검색 (Cloudflare Vectorize)
- 관리자 감사 로그 영속화 (계정·권한·검증 이력 — 계정 체계는 spec 0015 완료)
- 이미지 픽셀 단위 합성·딥페이크 탐지, 자동 마스킹(얼굴/번호판)
...
**관리자 페이지 (계정 인증 — spec 0015):**
1. 로그인 (`/admin/login`): username + password → JWT 세션. 비밀번호 설정 `/admin/reset?token=`(재설정 링크)
2. 검증 큐: 미검증 제보 목록
3. 상세 검증 뷰: 원본 첨부, AI 분석, 검증 메모
4. 대시보드: 검증 속도, 팀원 활동
5. 회원 관리 (`/admin/members`, 루트 전용): 검증자 계정 생성·비활성·삭제, 재설정 링크 발급

## 12. 추가 결정사항

**관리 및 운영:**
- ✅ 관리자 페이지 접근 제어 — 계정 인증 + 역할(루트/검증자), 재설정 링크 발급 (spec 0015). 남은 것: 이메일 자동 발송(현재 수동 전달), 감사 로그 영속화
