...
> 코드는 완성·검증(typecheck/test/static build)됐다. **운영 가동에 필요한 사람 작업**(운영 ADMIN_TOKEN 발급,
> 운영 D1 대상 export 실행 + GitHub 커밋 + Pages 재빌드, 선택적 Workers AI 바인딩)은 `loops/HUMAN.md` 참고.

**계획 (v1 이후):**
- 벡터 검색 (Cloudflare Vectorize)
- 멤버별 관리자 계정·권한·감사 로그 영속화 (현재 공유 토큰 MVP)
