...
> 비유: 요리할 때 **레시피를 먼저 쓰고** 요리를 시작하는 것과 같아요.
> "무엇을, 왜, 어떻게, 완료 조건은?"을 적은 종이(=스펙)를 먼저 만들고,
> 그걸 보고 코드를 짭니다. 다 만들면 레시피대로 됐는지 검사합니다.
...
```
specs/
  not-started/   ← 1. 레시피는 썼지만 아직 요리 안 함
  in-progress/   ← 2. 요리하는 중 (구현 중)
  in-review/     ← 3. 요리 끝, 검사 대기
  completed/     ← 4. 검사 통과 ✅
```
