...
이 프로젝트는 백엔드(API), 웹사이트, AI 검증 도구 세 부분으로 나뉘고,
셋 다 하나의 저장소(모노레포) 안에 있습니다.
...
├── apps/
│   ├── intake-api/       ← 백엔드 (제보 수집·검증 API)
│   ├── frontend/         ← 웹사이트 (제보 + 공개 아카이브)
│   └── admin-mcp/        ← AI 검증 도구 (MCP 서버)
├── specs/                ← 기능 설계 문서
├── steering/             ← 실수 노트 / 교훈
